...
export default function NewsSection({ darkMode }: NewsSectionProps) {
 ...
 useEffect(() => {
  ...
  if (isLoading) {
    return (
      <section className="max-w-[1600px] mx-auto px-8 mb-16">
        <h2 className="text-4xl font-bold mb-12 font-serif">Climate News</h2>
  ...
  return (
    <section className="max-w-[1600px] mx-auto px-8 mb-16">
      <h2 className="text-4xl font-bold mb-12 font-serif">Climate News</h2>
      <div className="grid gap-16 md:grid-cols-3 md:max-w-6xl mx-auto">
          ...
          {visibleArticles.map((article, index) => (
            ...
            >
              <Card 
                ...
                } flex flex-col w-full h-auto max-w-[380px] transition-all duration-300
                hover:shadow-2xl hover:-translate-y-2 hover:scale-[1.02]
                ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}`}
              >
                <div className="relative h-40 w-full overflow-hidden rounded-t-lg group">
                  <img
                    src={article.urlToImage || '/api/placeholder/800/600'}
                    alt={article.title}
                ...
                <CardHeader className="px-4 pt-4">
                  <CardTitle className="text-xl font-serif leading-tight mb-3 transition-colors duration-300 hover:text-blue-600 dark:hover:text-blue-400">
                    {article.title}
                  </CardTitle>
                ...
                <CardContent className="flex-grow px-4 pb-8">
                  ...
                  <a 
                    ...
                    className="inline-flex items-center text-blue-600 dark:text-blue-400 
                             font-medium group/link transition-colors duration-200"
                  >
                    <span className="border-b-2 border-transparent group-hover/link:border-current">
                      Read full story
                    </span>
                    <svg 
                      className="w-5 h-5 ml-2 transform transition-transform duration-200 group-hover/link:translate-x-1" 
                      viewBox="0 0 20 20" 
                      fill="currentColor"
                    >
